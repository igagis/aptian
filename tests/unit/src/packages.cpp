...
const tst::set set("packages", [](tst::suite& suite){ // NOLINT
    suite.add("parse_2_packages", [](){
        ...
        tst::check_eq(packages.size(), expected.size(), SL);
        tst::check_eq(packages[0].to_string(), expected[0].to_string(), SL);
        tst::check_eq(packages[1].to_string(), expected[1].to_string(), SL);
    });
});
}
